// Dafny program Group9DafnySpecification.dfy compiled into C#
...
predicate ValidItems(m: map<int, CartItem>)
  ...
  decreases {}, m
{
  forall id: int {:trigger m[id]} {:trigger id in m} :: 
    id in m ==>
      ValidCartItem(m[id])
}
...
class ShoppingCart {
  ...
  {
    total := 0;
    ghost var keyList := KeysToSeq(items.Keys);
    ghost var i := 0;
    while i < |keyList|
      invariant 0 <= i <= |keyList|
      invariant total >= 0
      invariant forall j: int {:trigger keyList[j]} :: 0 <= j < i ==> keyList[j] in items
      decreases |keyList| - i
    {
      ghost var id := keyList[i];
      if id in items {
        ghost var cartItem := items[id];
  ...
  ghost method KeysToSeq(keys: set<int>) returns (seqOut: seq<int>)
    ensures forall x: int {:trigger x in seqOut} {:trigger x in keys} :: x in keys ==> x in seqOut
    ensures forall x: int {:trigger x in keys} {:trigger x in seqOut} :: x in seqOut ==> x in keys
    ensures |seqOut| == |keys|
    decreases |keys|
  {
    if keys == {} {
      ...
      ghost var x := SetChoose(keys);
      ghost var tail := KeysToSeq(keys - {x});
      seqOut := [x] + tail;
    }
  }
}
...
namespace _module {
  ...
  public partial class __default {
    public static bool ValidCartItem(CartItem c) {
      return ((c.quantity).Sign == 1) && ((c.item.price).Sign != -1);
    }
    public static bool ValidItems(Dafny.IMap<BigInteger,CartItem> m) {
      return Dafny.Helpers.Id<Func<Dafny.IMap<BigInteger,CartItem>, bool>>((_0_m) => Dafny.Helpers.Quantifier<BigInteger>((_0_m).Keys.Elements, true, (((_forall_var_0) => {
        BigInteger _1_id = (BigInteger)_forall_var_0;
        return !((_0_m).Contains(_1_id)) || (__default.ValidCartItem(Dafny.Map<BigInteger, CartItem>.Select(_0_m,_1_id)));
      }))))(m);
    }
    public static void _Main(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      ...
      _nw2.__ctor();
      _2_cart = _nw2;
      (_2_cart).AddItem(_0_item1, BigInteger.One);
      (_2_cart).AddItem(_1_item2, new BigInteger(2));
      (_2_cart).ChangeQuantity(_1_item2, new BigInteger(3));
      if ((_2_cart.items).Contains(_0_item1.id)) {
        (_2_cart).DeleteItem(_0_item1);
      } else {
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Item1 is not in the cart.")).ToVerbatimString(false));
      }
      Dafny.IMap<BigInteger,CartItem> _3_snapshot;
      Dafny.IMap<BigInteger,CartItem> _out0;
      _out0 = (_2_cart).Checkout();
      _3_snapshot = _out0;
      Dafny.Helpers.Print((_System.Tuple2<Dafny.ISequence<Dafny.Rune>, Dafny.IMap<BigInteger,CartItem>>.create(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Cart snapshot after modifications: "), _3_snapshot)));
    }
  }
  ...
  public partial class Item {
    public Item() {
      ...
      this.name = Dafny.Sequence<Dafny.Rune>.Empty;
      this.price = BigInteger.Zero;
    }
    public BigInteger id {get; set;}
    public Dafny.ISequence<Dafny.Rune> name {get; set;}
    public BigInteger price {get; set;}
    public void __ctor(BigInteger id, Dafny.ISequence<Dafny.Rune> name, BigInteger price)
    {
      (this).id = id;
      (this).name = name;
      (this).price = price;
  ...
  public partial class CartItem {
    ...
    public void __ctor(Item item, BigInteger quantity)
    {
      (this).item = item;
      (this).quantity = quantity;
    }
  ...
  public partial class ShoppingCart {
    public ShoppingCart() {
      this.items = Dafny.Map<BigInteger, CartItem>.Empty;
    }
    public Dafny.IMap<BigInteger,CartItem> items {get; set;}
    public void __ctor()
    {
      (this).items = Dafny.Map<BigInteger, CartItem>.FromElements();
    }
    public void AddItem(Item item, BigInteger quantity)
    {
      if ((this.items).Contains(item.id)) {
        CartItem _0_existing;
        _0_existing = Dafny.Map<BigInteger, CartItem>.Select(this.items,item.id);
        BigInteger _1_newQ;
        _1_newQ = (_0_existing.quantity) + (quantity);
        CartItem _2_updated;
        CartItem _nw0 = new CartItem();
        _nw0.__ctor(item, _1_newQ);
        _2_updated = _nw0;
        (this).items = Dafny.Map<BigInteger, CartItem>.Update(this.items, item.id, _2_updated);
      } else {
        CartItem _3_newItem;
        CartItem _nw1 = new CartItem();
        _nw1.__ctor(item, quantity);
        _3_newItem = _nw1;
        (this).items = Dafny.Map<BigInteger, CartItem>.Update(this.items, item.id, _3_newItem);
      }
    }
    public void DeleteItem(Item item)
    {
      (this).items = Dafny.Map<BigInteger, CartItem>.Subtract(this.items, Dafny.Set<BigInteger>.FromElements(item.id));
    }
    public void ChangeQuantity(Item item, BigInteger newQuantity)
    {
      CartItem _0_existing;
      _0_existing = Dafny.Map<BigInteger, CartItem>.Select(this.items,item.id);
      CartItem _1_updated;
      CartItem _nw0 = new CartItem();
      _nw0.__ctor(_0_existing.item, newQuantity);
      _1_updated = _nw0;
      (this).items = Dafny.Map<BigInteger, CartItem>.Update(this.items, item.id, _1_updated);
    }
    public Dafny.IMap<BigInteger,CartItem> Checkout()
    {
      Dafny.IMap<BigInteger,CartItem> snapshot = Dafny.Map<BigInteger, CartItem>.Empty;
      snapshot = this.items;
      return snapshot;
    }
  }
